...
//        a($a);
            ...
            <div class="actionbuttons-bar">
                <?php if (true === $a['showActionButtons']): ?>
                    <?php foreach ($a['actionButtons'] as $id => $actionButton): ?>
                        <button data-id="<?php echo $id; ?>"><?php echo $actionButton['label']; ?></button>
                    <?php endforeach ?>
                <?php endif; ?>
            </div>
            <div class="top-bar">
                <?php if (true === $a['showNipp']): ?>
                    <div>
                        <?php echo DataTableRendererUtil::getNippSelector($a); ?>
                    </div>
                <?php endif; ?>
                <?php if (true === $a['showQuickPage']): ?>
                    <div>
                        <?php echo $a['textQuickPage']; ?>
                        <input type="text" value="<?php echo $a['page']; ?>">
                        <button><?php echo $a['textQuickPageButton']; ?></button>
                    </div>
                <?php endif; ?>
            </div>
            <table>
                ...
                <?php if (true === $a['isSearchable']): ?>
                    <tr class="search_row">
                        ...
                        <?php endforeach; ?>
                        <td>
                            <button><?php echo $a['textSearch']; ?></button>
                        </td>
                    </tr>
                <?php endif; ?>
                </thead>
                    ...
                    <?php foreach ($a['rows'] as $row): ?>
                        <tr>
                            ...
                            <?php if (true === $a['checkboxes']):
                                ...
                                ?>
                                <td><input type="checkbox" data-id="<?php echo $ricString; ?>"></td>
                            <?php endif; ?>
    ...
    protected function renderLink(array $data)
    {
        ...
        unset($data['label']);
        unset($data['icon']);
        return '<button' . DataTableRendererUtil::toDataAttributes($data) . '>' . $label . '</button>';
    }
